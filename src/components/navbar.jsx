...
export default function Navbar({ title, icons, position, hidden }) {
  ...
  return (
    <>
      <nav
        className={`flex items-center text-slate-300! ${position === "left" ? "justify-center" : "justify-between"} bg-primary fixed top-0
      left-0 w-full h-20 px-2.5 z-50`}
      >
        {icons}
        <h1 className="font-b text-slate-300 text-xl">{title}</h1>
      </nav>
